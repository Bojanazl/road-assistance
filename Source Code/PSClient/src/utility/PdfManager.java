...
public class PdfManager {
	...
	public static boolean exportPDF(Intervention intervention) {
		try {
			...
			String reportName = makeFolder() + File.separator
					+ TimeUtility.localDateToString(intervention.getReportTime().toLocalDate()) + "izvjestaj-"
					+ intervention.getId() + ".pdf";
			...
			document.add(data);
			Paragraph call = new Paragraph(
					"Poziv primljen:  " + TimeUtility.localDateTimeToString(intervention.getOpenedOn()));
			document.add(call);
			...
			document.add(v);
			Paragraph timeV = new Paragraph(
					"Vrijeme usluge:  " + TimeUtility.localDateTimeToString(intervention.getServiceTime()));
			document.add(timeV);
